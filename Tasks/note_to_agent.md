...
-   **E2E Testing for Programmatic Auto-Rotate**: The `AutoRotate` component does not expose a direct UI button. Its behavior is controlled programmatically by the `showSettings` URL parameter in `ModelViewerCore.tsx`. To test this, E2E tests should assert the presence or absence of the `AutoRotate` script component (`data-testid="auto-rotate-script"`) in the DOM based on the `settings` URL parameter. Visual verification (e.g., screenshots) can optionally be used to confirm actual rotation.

## 17. E2E Testing for `react-range-slider-input` components

-   **Challenges with Locators**: Standard Playwright locators like `page.getByRole('slider')` or `page.locator('input[type="range"]')` and `fill()` method were ineffective for `react-range-slider-input` components. This is because the library renders a custom DOM structure (primarily `div` elements with specific classes) rather than standard HTML input elements for its sliders, and its accessibility roles might not be fully exposed or recognized by Playwright's accessibility tree.
-   **Solution: XPath Locators and Mouse Drag Actions**: The most reliable approach found was to:
    1.  **Locate the parent container** of the slider using text content (e.g., `page.locator('label', { hasText: 'Position X' })` for single sliders, or `page.locator('h3', { hasText: 'Camera Distance Settings' })` for dual sliders).
    2.  **Use XPath to find the `RangeSlider`'s root `div`** (which has class `range-slider`) relative to its unique parent/sibling elements (e.g., `labelLocator.locator('xpath=./following-sibling::div[contains(@class, "range-slider")]')` or `h3Locator.locator('xpath=./following-sibling::div[1]/following-sibling::div[contains(@class, "range-slider")]')`).
    3.  **Locate the draggable thumbs** within the `range-slider` div using their class (e.g., `.range-slider__thumb`).
    4.  **Simulate mouse drag actions** (`page.mouse.move`, `page.mouse.down`, `page.mouse.up`) on the thumbs to change their values. Fixed pixel movements (e.g., `+5` or `-5` pixels) were more reliable than attempting to map values to precise pixel coordinates, as the component's internal value-to-pixel mapping might be non-linear or sensitive to minor variations.
    5.  **Assert value changes** by reading the updated text content from the associated labels and parsing the numerical values. Assertions should check that values have changed from their initial state and remain within the expected min/max range of the slider.
-   **Recommendation**: For future E2E tests involving `react-range-slider-input` or similar custom slider components, prioritize mouse drag actions with relative pixel movements and robust XPath locators over direct `fill()` or `getByRole` methods.

## 18. E2E Testing PlayCanvas Components and `data-testid` Consistency

-   **Challenges with PlayCanvas Component Assertions in Playwright**: PlayCanvas React components (e.g., `Script`, `Entity`) do not render directly into the DOM in a way that Playwright can reliably assert on their visibility or presence using standard methods like `toBeVisible()` or `toHaveCount()`. This is because they primarily interact with a WebGL canvas, not the traditional DOM.
-   **Solution: Indirect Assertion via `data-` Attributes**: To effectively test conditional rendering or state changes of PlayCanvas components in E2E tests, it's recommended to:
    1.  Add a `data-` attribute (e.g., `data-grid-visible`) to a visible parent DOM element (e.g., the settings panel in `ModelViewerCore.tsx`) that reflects the internal state controlling the PlayCanvas component's rendering.
...
-   **`data-testid` Consistency**: It is crucial to maintain consistency of `data-testid` attributes across component implementations, unit tests (Vitest), and E2E tests (Playwright). If a `data-testid` is changed in a component, ensure all corresponding unit and E2E tests are updated to reflect the new `data-testid` to prevent test failures. This was observed when updating `Grid.tsx` and `Grid.test.tsx`.
